// public/js/productManagement.js
document.addEventListener("DOMContentLoaded", function () {
    ...
    newCategoryInput.addEventListener("keypress", function (e) {
      if (e.key === "Enter" && this.value.trim() !== "") {
        ...
        categorySelect.value = newCategory; // Set the selected value
        this.classList.add("hidden");
        // Keep the value in newCategory input for form submission
      }
    });
    ...
    form.addEventListener("submit", async function (e) {
      ...
      if (!inputs.image.files.length) {
        errors.image.textContent = "Image is required.";
        errors.image.classList.remove("hidden");
        allFilled = false;
      }
  
      if (!isValid || !allFilled) return;
  
      const formData = new FormData(form);
  
      // If newCategory was entered, ensure it’s prioritized
      const newCategoryValue = newCategoryInput.value.trim();
      if (newCategoryValue !== "") {
        formData.set("category", newCategoryValue); // Override category with newCategory
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          newCategoryInput.value = ""; // Clear after successful submission
