...
exports.updateCart = async (req, res) => {
  ...
  try {
    ...
    // Map updated items and check stock
    const updatedCartItems = cart.items.map(item => {
      ...
      const availableQuantity = item.product.quantity; // From Product model
      ...
      return {
        ...
        availableQuantity: availableQuantity // Add stock info for frontend
      };
    });
    ...
    if (stockIssues.length > 0) {
      return res.status(400).json({
        ...
        cart: { items: updatedCartItems }, // Return cart with stock info
    ...
    // If no stock issues, update cart items
    ...
    // Update totals (assuming updateCartTotals is defined elsewhere)
    await updateCartTotals(cart);
    await cart.save();

    // Prepare response
    const cartResponse = cart.toObject();
    cartResponse.items = updatedCartItems; // Include availableQuantity in response
